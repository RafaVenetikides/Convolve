...
struct PlaygroundCollectionView: View {
    ...
    private enum Focus: Hashable { case title }
    ...
    private let columns = [
        GridItem(.adaptive(minimum: 200), spacing: 150)
    ]
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                Text(
                    "Pick an example to apply kernels and watch the scan in convolution in real time."
                )
                .font(.system(size: 24))
                ...
                ScrollView {
                    LazyVGrid(columns: columns, spacing: 30) {
                        let isLoading = isLoadingPhoto
                        PhotosPicker(
                            selection: $pickedItem,
                            matching: .images,
                            photoLibrary: .shared()
                        ) {
                            PhotoPickerCard(isLoading: isLoading)
                        }
                        .buttonStyle(.plain)
                        .disabled(isLoading)
                        ...
                        ForEach(assets) { item in
                            Button {
                                router.push(
                                    .convolution(assetName: item.assetName)
                                )
                            } label: {
                                AssetCard(item: item)
                            }
                            ...
                            .accessibilityHint(
                                "Opens the convolution playground with this image."
                            )
                            .accessibilityAddTraits(.isButton)
                        }
                    }
        ...
        .onChange(of: pickedItem) { _, newItem in
            ...
            Task {
                ...
                if let data = try? await newItem.loadTransferable(
                    ...
                    print(newImage.size)
                    router.push(
                        .convolutionPhoto(imageData: newImage.pngData()!)
                    )
                }
